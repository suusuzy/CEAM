...
class EnhancedCrossAttention(nn.Module):
    def __init__(self, d_model=64, nhead=8, dropout=0.1):
        ...
        self.cross_attn = MultiheadAttention(
            ...
            num_heads=nhead,
            dropout=dropout
            #batch_first=False  # PyTorch的MHA需要(N,T,C)格式
        )
    ...
    def forward(self, dyna_out, gat_out):
        """
        Args:
        ...
        """
        # 1. 特征投影
        dyna_emb = self.dyna_proj(dyna_out)  # [T,N,d_model]
        gat_emb = self.gat_proj(gat_out)     # [T,N,d_model]
        
        # 2. 调整维度格式 (PyTorch MHA需要[N,T,C])
        query = dyna_emb.permute(1, 0, 2)  # [N,T,d_model]
        key_value = gat_emb.permute(1, 0, 2)  # [N,T,d_model]
        ...
        attn_output = self.norm1(attn_output)  # [N,T,d_model]
        
        # 5. 前馈网络
        ffn_output = self.ffn(attn_output)
        ffn_output = attn_output + self.dropout(ffn_output)
        ffn_output = self.norm2(ffn_output)  # [N,T,d_model]
        
        # 6. 恢复维度并输出
        output = ffn_output.permute(1, 0, 2)  # [T,N,d_model]
        
        # 7. 柔性残差融合
        return self.alpha * self.out_proj(output) + (1-self.alpha) * dyna_out
...
class LinearAttentionFusion(nn.Module):
    def __init__(self, feature_dim=2):
        super(LinearAttentionFusion, self).__init__()
        # 线性变换层，输出注意力权重
        self.attention = nn.Sequential(
            nn.Linear(feature_dim * 2, 1),  # 输入是拼接的两个特征
            nn.Sigmoid()  # 输出0-1之间的权重
        )
        
    def forward(self, dyna_output, gat_output):
        """
        Args:
            dyna_output: [T, N, 2]
            gat_output: [T, N, 2]
        Returns:
            fused_output: [T, N, 2]
        """
        T, N, _ = dyna_output.shape
        
        # 拼接两个特征
        combined = torch.cat([dyna_output, gat_output], dim=-1)  # [T, N, 4] 8
        
        # 计算注意力权重
        alpha = self.attention(combined)  # [T, N, 1]
        
        # 加权融合
        fused_output = alpha * dyna_output + (1 - alpha) * gat_output
        
        return fused_output

class DYNAWithGAT(nn.Module):
    def __init__(self, args, dropout_prob=0):
        super(DYNAWithGAT, self).__init__()
        self.args = args
        self.dropout_prob = dropout_prob
        self.K = 1
        ...
        self.gat = GATv2Conv(2, 8, add_self_loops=False)#[4*4=16]
        ...
        # 线性注意力融合
        # self.fusion = LinearAttentionFusion()
        self.fusion = EnhancedCrossAttention(d_model=32, nhead=8)
