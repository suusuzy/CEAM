...
class Trajectory_Dataloader():
    ...
    def reset_batch_pointer(self, set, valid=True):
        '''
        Reset all pointers
        '''
        if set == 'train':
...
def getLossMask(outputs, node_first, seq_list, using_cuda=False):
    ...
    for framenum in range(seq_length):
        ...
        if framenum == 0:
            lossmask_list.append(seq_list[framenum] * node_pre)
        else:
            lossmask_list.append(seq_list[framenum] * lossmask_list[framenum - 1])

    lossmask = torch.stack(lossmask_list, dim=0)  # 将列表转换为张量

    if using_cuda:
        lossmask = lossmask.cuda()

    return lossmask, lossmask.sum()
